...
func (s *Service) Overview(ctx context.Context) (*Overview, error) {
	...
	// 超期预警：分阶段口径（未开工 / 未报验 / 未验收），与预警列表、任务列表一致
	overdueTotal, overdueByStage, err := s.overdue.CountByStage(ctx)
	if err != nil {
		return nil, err
	}
	result.TaskOverdue = overdueTotal
	result.TaskOverdueByStage = overdueByStage
	...
	monthStart := date.New(time.Date(date.Today().Year(), date.Today().Month(), 1, 0, 0, 0, 0, time.UTC))
...
//
// OverdueDays 采用预警模块「未验收」阶段口径：报验后等待验收的天数超过阈值才计为超期。
func (s *Service) PendingAcceptance(ctx context.Context, limit int) ([]PendingAcceptanceItem, error) {
	...
	for i := range items {
		...
		waitDays := overdue.OverdueDays(overdue.StageNotAccepted, overdue.Snapshot{FinishedAt: items[i].FinishedAt}, today)
		if waitDays > thresholds.NotAcceptedDays {
			items[i].OverdueDays = waitDays
		}
	}
	return items, nil
}
